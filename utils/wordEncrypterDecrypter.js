...
export function encryptSentence(sentence) {
  ...
  const encrypted = CryptoJS.AES.encrypt(sentence, "3Tcq_dBhIrQ").toString();
...
// Decryption function
export function decryptSentence(encryptedSentence) {
  // Convert from URL-safe base64 to base64
  const base64Encoded = decodeUrlSafeToBase64(encryptedSentence);

  // Decrypt the base64 encoded string
  const bytes = CryptoJS.AES.decrypt(base64Encoded, "3Tcq_dBhIrQ");
